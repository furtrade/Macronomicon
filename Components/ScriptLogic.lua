...
function addon:processButtons(type, info)
    ...
    if not self:buttonExists(name) then
        self:fetchButtonInfo(info, type)
    end
    self:updateButtonState(info)
end

-- Function to create a script button
function addon:fetchButtonInfo(info, type)
    local macroText = self:buildText(info)
    local buttonData = {
        name = info.name,
        macroText = macroText,
        tooltip = info.tooltip or info.name or "wtf",
...
function addon:processCustomScripts()
    for _, scriptInfo in pairs(self.macroData) do
